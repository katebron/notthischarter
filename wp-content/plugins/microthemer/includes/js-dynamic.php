...
$data.= 'TvrCombo.custom_paths = ' . json_encode($this->preferences['custom_paths']) . ';' . "\n\n";

// ready combo for MQ and CSS unit sets (dynamic so they can be translated)
foreach ($this->mq_sets as $set => $junk){
	$mq_sets[] = $set;
}
$data.= 'TvrCombo.mq_sets = ' . json_encode($mq_sets) . ';' . "\n\n";
// $data.= 'TvrCombo.css_unit_sets = ' . json_encode($this->css_unit_sets) . ';' . "\n\n";
